...
semantic-cve/
├── apps/
│   ├── web/          # Next.js 15 frontend (Tailwind v4, shadcn/ui)
...
The web frontend is a dark-themed, shadcn/ui-based application styled with Tailwind CSS v4 and the Geist font family (Sans + Pixel). It communicates with the API via `fetch` calls.

**Pages:**

| Route | Purpose |
|---|---|
| `/` | Hero search bar with example query chips, feature labels |
| `/search?q=&mode=` | Search results with CVE cards, score bars, pagination, mode selector, loading skeletons, empty/error states |
| `/cve/:id` | CVE detail with metadata cards, CVSS score display, similar CVEs, references list, raw JSON viewer |

**Key components:**
- `Header` — Sticky branded header with search bar (on non-home pages), GitHub link, mobile sheet drawer
- `Footer` — Minimal footer with copyright
- `CveCard` — Reusable CVE result card with severity badge, score bar, vendor/product info
- UI primitives from shadcn (`Button`, `Input`, `Card`, `Badge`, `Tabs`, `Pagination`, `Sheet`, `Dialog`, `Separator`, `Skeleton`) built on `@base-ui/react`

**Dependencies:** `next@15`, `react@19`, `tailwindcss@4`, `@base-ui/react`, `lucide-react`, `geist`, `shadcn`, `class-variance-authority`, `tailwind-merge`

```bash
bun run --filter @semantic-cve/web dev     # development on :3000
bun run --filter @semantic-cve/web build   # production build
```
...
|---|---|---|
| Frontend | **Vercel** | Zero-config Next.js deployment. Set `NEXT_PUBLIC_API_URL` to your API endpoint |
| API | **Small VPS** ($5-10/mo) | Requires Ollama + SQLite. 2GB RAM minimum for embedding model |
...
|---|---|---|
| `DATABASE_URL` | — | Not used (SQLite is file-based at `cve-cache/db/cve.db`) |
...
| Styling | [Tailwind CSS](https://tailwindcss.com) v4, shadcn/ui, Geist fonts |
